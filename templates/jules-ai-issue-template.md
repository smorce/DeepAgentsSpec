...
5. **ドメイン別チェックリスト（必要に応じて追加）**  
   - UX/セキュリティ/パフォーマンスなど追加観点が必要なら、`scripts/bash/add-domain-checklist.sh`（Windows は `scripts/powershell/add-domain-checklist.ps1`）で `templates/checklist/` からテンプレを `FEATURE_DIR/checklists/<slug>.md` に生成し、Issue 内にパスと目的を明記する。
...
- **Checklists**  
  - `requirements.md`、`PlanQualityGate.md` に加え、`add-domain-checklist.*` で生成した追加ドメインチェックリスト（例: `checklists/security.md`）の完了状況と対応ログを Issue 本文にまとめる。
- **ログ提出**  
...
- [ ] 追加したドメインチェックリスト（例: `checklists/security.md`）を最新化し、未完了項目が残る場合は理由とフォローアップを Issue に記載
