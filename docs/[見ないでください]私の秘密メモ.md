...
```
curl -X POST http://localhost:8165/minirag/documents/bulk \             
-H "Content-Type: application/json" \
-d '{"documents":[{"workspace":"procurement_docs","doc_id":"proc-plan-
fy2026-apac","title":"タイトル","summary":"要約","body":"本             
文","status":"in_progress","region":"APAC","priority":1,"metadata":     
{"category":"planning_document"}}],"overwrite":true}'

curl -X POST http://localhost:8165/minirag/search \
-H "Content-Type: application/json" \
-d '{"query":"調達計画について教えて","modes":                        
["mini","light"],"top_k":3,"include_provenance":true}'
```

簡易チャットUIの方はCodex Webで実装して、プルリクまで作成した。マージはまだ。
